...
mod tests {
    ...
    const ENV_STM32_CUBE_PROGRAMMER_DOWNLOAD_HEX_PATH: &str =
        "STM32_CUBE_PROGRAMMER_DOWNLOAD_HEX_PATH";

    /// The path to a .bin file for downloading to the target
    const ENV_STM32_CUBE_PROGRAMMER_DOWNLOAD_BIN_PATH: &str =
    ...
    fn get_address_from_env_file(env_key: &str) -> u32 {
        dotenvy::dotenv().unwrap();
        let hex_string =
            std::env::var(env_key).expect(&format!("{} not found in .env file", env_key));
    ...
    fn download_hex_file() {
        ...
        if !probes.is_empty() {
            ...
            connected_programmer
                .reset_target(api_types::ResetMode::HardwareReset)
                .unwrap();
            connected_programmer.disconnect();
        } else {
            info!("No ST-Link probes found");
    ...
    fn register_display_handler() {
        ...
        impl crate::DisplayCallback for MyDisplayHandler {
            fn init_progressbar(&self) {
                warn!("MyDisplayHandler - Init progress bar");
            }

            fn log_message(&self, message_type: crate::LogMessageType, message: &str) {
                info!(
